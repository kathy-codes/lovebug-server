CREATE TABLE `users` (
    ...
    `career_ambition` float DEFAULT NULL,
...
-- Create initial matches using the newly generated UUIDs
INSERT INTO `matches` (`user_a_id`, `user_b_id`, `compatibility_score`, `is_compatible`, `relationship_longevity_months`) VALUES 
((SELECT user_id FROM users WHERE name = 'Liam'), (SELECT user_id FROM users WHERE name = 'Sophia'), 43.5, 0, 60),
((SELECT user_id FROM users WHERE name = 'Ethan'), (SELECT user_id FROM users WHERE name = 'Isabella'), 74.3, 1, 84);